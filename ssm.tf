...
resource "aws_ssm_parameter" "database_url" {
  ...
  type        = "SecureString"
  value       = "postgres://myuser:${data.aws_kms_secrets.secrets.plaintext["db_password"]}@${aws_rds_cluster.main.endpoint}:5432"
}
...
resource "aws_ssm_parameter" "reader_database_url" {
  ...
  type        = "SecureString"
  value       = "postgres://myuser:${data.aws_kms_secrets.secrets.plaintext["db_password"]}@${aws_rds_cluster.main.reader_endpoint}:5432"
}
